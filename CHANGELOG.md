...
- 新增 `pipeline.py package <文章.md>`：直接生成封面、两张正文信息图、Obsidian 图文成稿、公众号/小红书/即刻文案。
- 新增 `pipeline.py check <发布包目录>`：按 `manifest.json` 校验所有图片和平台文案，缺图或素材被改动会失败。
- 图片生成改为本地、确定性的 Pillow 信息图，不再要求 Agent 或 Codex Harness 调用外部出图工具。
- 小红书和即刻改为诚实的手动发布包；不再把浏览器自动化当作默认能力。
- 新增最小依赖 `requirements.txt` 与端到端单元测试。

### 兼容

- 旧版飞书审核和 stage 命令继续保留，但不属于 v3 默认流程。
